        ...
        public Task<ResponseOPDNGetPO> responseOPDNGetPO()
        {
            ...
            try
            {
                ...
                if (login.LErrCode == 0)
                {
                    ...
                    string sqlStr = "CALL \"" + ConnectionString.CompanyDB + "\"._USP_GETMASTER('OPDN','','','','','')"; ;
                    ...
                    while (!oRS.EoF)
                    {
                        ...
                        oRSLine.DoQuery("CALL \"" + ConnectionString.CompanyDB + "\"._USP_GETMASTER('PDN1','"+ oRS.Fields.Item(0).Value + "','','','','')");
                        pDN1s = new List<PDN1>();
                        while (!oRSLine.EoF)
                        {
                            pDN1s.Add(new PDN1
